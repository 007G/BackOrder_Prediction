...
class File_Operation:
    ...
    def save_model(self,model,filename):
        ...
        try:
            ...
            self.logger_object.log(self.logfile,'Model File '+filename+' saved. Exited the save_model method of the Model_Selector class')
            self.logfile.close()
            return 'success'

        except Exception as e:
            self.logger_object.log(self.logfile,'Exception occured in save_model method of the Model_Selector class. Exception message:  ' + str(e))
            self.logger_object.log(self.logfile,'Model File '+filename+' could not be saved. Exited the save_model method of the Model_Selector class')
            self.logfile.close()
            raise Exception()
    ...
    def load_model(self,filename):
        ...
        try:
            with open(self.model_directory + filename + '/' + filename + '.sav','rb') as f:
                self.logger_object.log(self.logfile,'Model File ' + filename + ' loaded. Exited the load_model method of the Model_Selector class')
                self.logfile.close()
                return pickle.load(f)

        except Exception as e:
            self.logger_object.log(self.logfile,'Exception occured in load_model method of the Model_Selector class. Exception message:  ' + str(e))
            self.logger_object.log(self.logfile,'Model File ' + filename + ' could not be saved. Exited the load_model method of the Model_Selector class')
            self.logfile.close()
            raise Exception()
